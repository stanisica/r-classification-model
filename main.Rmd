...
```{r eval=T, results='hold'}
location_desc_counts <- data_clean %>%
...
ggplot(location_desc_counts, aes(x = reorder(Location_Description, -count), y = count)) +
  geom_bar(stat = "identity", fill = "#595959") +
  labs(title = "Frequency of Location Descripiton",
       x = "Primary Type",
       y = "Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 5),
...
```{r eval=T, results='hold'}
community_crime_rates <- community_data_modified %>%
  group_by(Community_Area) %>%
  summarise(Num_Crimes = n())

print(community_crime_rates)

community_crime_rates <- community_crime_rates %>%
  mutate(Crime_Rate = Num_Crimes / sum(Num_Crimes))

ggplot(data = community_crime_rates, aes(x = Community_Area, y = Crime_Rate)) +
  geom_point() +
  geom_text(aes(label = round(Crime_Rate, 3)), vjust = -0.5) +
  geom_line() +
  labs(x = "Административна јединица", y = "Стопа криминала", title = "Стопа криминала агрегираних административних јединица") +
  theme_minimal() 
```
...
```{r eval=T, results='hold'}
arrest_cost_matrix <- matrix(c(1, 1, 3, 1), nrow = 2)
...
prepared_data <- weighted_data %>%
  select(Community_Area, Domestic, Primary_Type, Beat, Ward, Weight, Arrest, Year, IUCR, Location_Description, FBI_Code)
