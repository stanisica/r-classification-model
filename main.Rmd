...
-   Визуализација односа криминалних дела који спадају у групу :
```{r eval=T, results='hold'}
domestic_arrest_data <- community_data_modified  %>%
  select(Domestic, Arrest)

ggplot(domestic_arrest_data, aes(x = Domestic, y = Arrest)) +
  geom_point() +
  labs(title = "Odnos između Domestic i Arrest",
       x = "Domestic", y = "Arrest")
```
...
randf_params <- list(
  random_forest_classifier = list(num_trees = c(15, 25, 60))
)
...
## Упоредна анализа класификационих модела
